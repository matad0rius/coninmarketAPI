...
file = open('Portofolio.txt', 'r')

Symbols = []
Quantitys = []
Buy_prices = []

for line in file.readlines()[1:]: # not read the 1st line 
    line = line.split(', ') 
...
def color(number):
    if number > 0:
        return ('\033[92m' + str(number) + '\033[0m') # green
    else:
        return ('\033[91m' + str(number) + '\033[0m') # red
...
for i in range(len(Symbols)):
    ...
    buy_value += Buy_pricees[i] * Quantitys[i]
    current_value += current_price * Quantitys[i]
    table.add_row([Symbols[i], Quantitys[i], Buy_prices[i], current_price, color(profit),
                    color(change_1g), color(change_1d), color(change_7d)])
                    
print(table)

total_profi = current_value / buy_value * 100 - 100
dolars = current_value - buy_value

print("Portofolio Value: %.2f  Total Profit: %%: %.2f  Total Profit USD: %.2f" % (current_value, total_profit, dolars))
